...
class TestAdaptAsgiRequest:
    ...
    async def test_chunked_body_reception(self):
        ...
        chunks = [
            full_body[i : i + chunk_size] for i in range(0, len(full_body), chunk_size)
        ]
        ...
        for i, chunk in enumerate(chunks):
            ...
            receive_calls.append(
                {"type": "http.request", "body": chunk, "more_body": not is_last}
            )
    ...
    async def test_timeout_error_handling(self):
        ...
        scope = {"type": "http", "method": "GET", "path": "/api/timeout", "headers": []}
    ...
    async def test_missing_scope_defaults(self):
        ...
        receive = AsyncMock(
            return_value={"type": "http.request", "body": b"test", "more_body": False}
        )
        ...
        assert (
            result.body == b"test"
        )  # Should still process body as binary (no content-type)
        assert result.source == "knative"
